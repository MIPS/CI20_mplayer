...
#include "t_vlc_sde_aux.c"
#include "h264_cavlc_p1_dblk.c"
...
__p1_main int main() {
  ...
  *wfifo_ptr = vmau_glb_idx;   
  ...
  //int mpframe = read_reg(TCSM1_END,0x0);
  volatile unsigned int *gp2_dha = GP2_DHA_BASE;
  volatile unsigned int *gp2_index_ptr = GP2_INDEX_BASE;
  unsigned int gp2_dha_ptr = GP2_DHA_VEC;
  ...
  *((volatile unsigned int*)motion_dha) = 0 ;
  ...
  vlc_cfg_sde(sde_mb_x, mb_num, mb_width,neighbor_info,sde_ctrl_dout_caddr,mau_main_src_addr,aux_sde_top_info_ptr);
  *((volatile int*)(SDE_PBASE + SDE_DESPC_OFST)) = ((aux_do_get_phy_addr(neighbor_info)) & 0xfffffffc) + 1;
  ...
  do {
    ...
    vlc_cfg_sde(sde_mb_x, mb_num, mb_width,next_neighbor_info,next_sde_ctrl_dout_caddr,next_mau_main_src_addr,aux_sde_top_info_ptr);
    ...
    do {
      ...
      if (sde_err) goto error;
      sde_end = *sde_sync_end;
    } while (sde_end == 0);
    *sde_sync_end = 0;

    if(++last_mb_x >= mb_width){
      ++last_mb_y;
      if(last_mb_y >= slice_info_ptr->mb_height){ 
	aux_sde_mb_terminate = 1;
        goto end;
      }
    }
    if((mb_intra->bit_count+slice_info_ptr->bscnt_or_ref1cnt)>=slice_info_ptr->sbits_or_ref0cnt && mb_intra->skip_run<=0){
      aux_sde_mb_terminate = 1;
    }
...
{                                                      \
  for(blk_i=0; blk_i<2; blk_i++)                       \
	...
	short *mvp[2];
	mb_inter_p_vlc_p ptr = sde_ctrl_dout_caddr;
	if( slice_type == SDE_P_TYPE ){
	  mvp[0] = ptr->mv;
	} else {
	  mvp[0] = ((mb_inter_b_vlc_p)ptr)->mv;
	  mvp[1] = &((mb_inter_b_vlc_p)ptr)->mv[ptr->mv_len>>1];
	}
    ...
    *qp_ptr += mb_intra->qscal_diff;
    ...
    vlc_hw_dblk_mb_aux_c(mb_num,sde_ptr,slice_type,mb_x,mb_y,y_qp,u_qp,v_qp,dblk_y_chn_addr,dblk_uv_chn_addr,
			 last_glb_idx,&aux_sde_top_info_ptr[mb_x],dblk_mv_addr,h264_depth_offset,deblock_filter);

    backup_sde_vlc_aux(mb_x,sde_ctrl_dout_caddr,aux_sde_top_info_ptr);
    ...
    unsigned int gp2_out_l_uv_ptr =(lc - 128) + 4;    
    {
      ...
      gp2_dha[13] = gp2_out_up_y_ptr;
      gp2_dha[17] = gp2_out_up_uv_ptr;
      gp2_dha[21] = gp2_out_up_uv_ptr + 8;
	...
	gp2_dha[3] = GP_GSC(0,16,16*16);
	gp2_dha[7] = GP_GSC(0,8,8*8);    
	gp2_dha[11] = GP_GSC(0,8,8*8);
      }else{
        gp2_dha[3] = GP_GSC(0,16,16*12);
	gp2_dha[7] = GP_GSC(0,8,8*4);    
	gp2_dha[11] = GP_GSC(0,8,8*4);
      } 
