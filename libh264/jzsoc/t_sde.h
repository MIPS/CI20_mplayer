...
//#define write_reg(addr, val) ({ cmd_core_write(cmd_core_thread_ptr, (addr), (val)); })
//#define read_reg( reg , off) ({ cmd_core_read( cmd_core_thread_ptr, ( reg + off )); })
...
struct SLICE_INFO sde_slice_info;
struct SLICE_INFO *sde_slice_info_ptr;
struct NEIGHBOR_INFO sde_left_info;
struct NEIGHBOR_INFO sde_top_info[MB_W_1080P];
struct NEIGHBOR_INFO sde_top_info_tmp;

int * curr_slice_info_ctrl;
int * curr_slice_info_mv;
int slice_mv_cnt;

int frm_info_slice_start_mb_init;
int slice_num_sde;
int frm_info_mv_cnt;
int mb_num_sde;
int mx_sde;
