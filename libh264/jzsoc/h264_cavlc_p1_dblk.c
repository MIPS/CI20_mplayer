...
void vlc_hw_dblk_mb_aux_c(int mbnum, unsigned int *sde_ptr,int slice_type,
  ...
  mb_inter_p = (struct MB_INTER_P_VLC *)(sde_ptr);
  mb_inter_b = (struct MB_INTER_B_VLC *)(sde_ptr);
  ...
  int k,nnz_right = 0, nnz_bottom = 0;
  for(k=0; k<4; k++){
    nnz_bottom |= (((aux_sde_top_info_ptr->nnz_cbp & (0xF<<(4*k))) ? 1 : 0)<<k);
  }

  int nnz_br = nnz_bottom & 0xF;
  ...
  int nnz = (nnz_dct | (nnz_br<<16) | (nnz_left_dblk << 20));

  for(k=0; k<4; k++){
    nnz_right |= (((mb_inter_p->nnz_right & (0xF<<(4*k))) ? 1 : 0)<<k);
  }
  nnz_left_dblk = nnz_right&0xF;
