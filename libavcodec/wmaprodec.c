...
static int decode_frame(WMAProDecodeCtx *s)
{
    ...
    if(s->num_channels == 6){
      for (i = 0; i < 2; i++) {
        ...
        int32_t* iptr = s->channel[1].out;
        int32_t* iend = iptr + s->samples_per_frame;
        // FIXME should create/use a DSP function here
        while (iptr < iend) {
	    *ptr = ((float)(*iptr++))/268435456;
            ptr += 6;
        }
      }
        /** reuse second half of the IMDCT output for the next frame */
      memcpy(&s->channel[1].out[0],
               &s->channel[1].out[s->samples_per_frame],
               s->samples_per_frame * sizeof(*s->channel[1].out) >> 1);      
    }else{
       for (i = 0; i < s->num_channels; i++) {
        float* ptr  = s->samples + i;
        int incr = s->num_channels;
