...
#define ROA   256
	...
	if(use_jz_buf){
#ifdef JZC_TLB_OPT
	  picture.linesize[3] = picture.linesize[1];
	  int total_size;
	  int aline[4];
	  for(i=0; i<4; i++){
...
#if 0
	    if (i == 3) buf->linesize[i]= picture.linesize[1];
	    if (i == 2) buf->linesize[i]= picture.linesize[0];
#else
	    if (i == 3) buf->linesize[i]= 256;
	    if (i == 2) buf->linesize[i]= 256;
#endif
	    if (i==1) aline[i] = aline[0];            
...
#if 1
            if (i>1) total_size = 256;
#endif //not use rota   
	    buf->base[i] = jz4740_alloc_frame (256, total_size+16);
    
	    if(!tlb_i){
	      SET_VPU_TLB(0, 1, PSIZE_32M, get_phy_addr((int)buf->base[i])>>22, 
			  get_phy_addr((int)buf->base[i])>>22);
	      tlb_i++;
	      if(((GET_VPU_TLB(0)>>25) & 0x3FF) != ((get_phy_addr((int)buf->base[i]+total_size*3)>>25) & 0x3FF)){
		SET_VPU_TLB(tlb_i, 1, PSIZE_32M, get_phy_addr((int)buf->base[i]+total_size*3)>>22, 
			    get_phy_addr((int)buf->base[i]+total_size*3)>>22);
		tlb_i++;
	      }
	    }else {
	      int tmp_i, start_hit=0, end_hit=0;
	      for(tmp_i=0; tmp_i<tlb_i; tmp_i++){
		if(((GET_VPU_TLB(tmp_i)>>25) & 0x3FF) == ((get_phy_addr((int)buf->base[i])>>25) & 0x3FF) ){
		  start_hit++;
		  break;
		}
	      }
	      if(!start_hit){
		SET_VPU_TLB(tlb_i, 1, PSIZE_32M, get_phy_addr((int)buf->base[i])>>22, 
			    get_phy_addr((int)buf->base[i])>>22);
		...
		tlb_i++;
	      ...
	      for(tmp_i=0; tmp_i<tlb_i; tmp_i++){
		if(((GET_VPU_TLB(tmp_i)>>25) & 0x3FF) == ((get_phy_addr((int)buf->base[i]+total_size*3)>>25) & 0x3FF) ){
		  end_hit++;
		  break;
		}
	      }
	    ...
	    if((s->flags&CODEC_FLAG_EMU_EDGE) || !size[2])
	      ...
	      buf->data[i] = buf->base[i] + FFALIGN((aline[i]*EDGE_WIDTH>>v_shift) + (ROA>>v_shift) + (EDGE_WIDTH>>h_shift), ROA);}
#else
            if(((s->flags&CODEC_FLAG_EMU_EDGE) || !size[2]) && (i < 2))
	      buf->data[i] = buf->base[i];
	    else{
	      buf->data[i] = buf->base[i] + FFALIGN((aline[i]*EDGE_WIDTH>>v_shift) + (ROA>>v_shift) + (EDGE_WIDTH>>h_shift), ROA);}    
            buf->data[2] = buf->base[2];  
	    buf->data[3] = buf->base[3];  
#endif  //nouse rota
	  }
#else
    ...
    for(i=0; i<4; i++){
        ...
        pic->linesize[i]= buf->linesize[i]<<(use_jz_buf?4:0);
