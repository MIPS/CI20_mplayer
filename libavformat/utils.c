...
#if 0
	if(CODEC_ID_H264 == st->codec->codec_id ){
	  st->codec_info_nb_frames++;
          count++;
	  #undef printf
	  printf("FIXME: st->codec->codec_id = CODEC_ID_H264: break; do not run  try__decode_frame\n");
	  break;
	}
#endif// FIXME:figure out when &&why need to try to decode frame; by gjiwang 
